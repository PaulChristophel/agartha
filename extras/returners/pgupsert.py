...
:maintainer:    Paul Christophel <https://github.com/PaulChristophel>
:maturity:      New
:depends:       psycopg2
:platform:      all
...
.. code-block:: yaml
    ...
    returner.pgupsert.host: 'salt'
    returner.pgupsert.user: 'salt'
    returner.pgupsert.passwd: 'salt'
    returner.pgupsert.db: 'salt'
    returner.pgupsert.port: 5432
...
.. code-block:: yaml
    ...
    alternative.returner.pgupsert.host: 'salt'
    alternative.returner.pgupsert.user: 'salt'
    alternative.returner.pgupsert.passwd: 'salt'
    alternative.returner.pgupsert.db: 'salt'
    alternative.returner.pgupsert.port: 5432
...
.. code-block:: sql
    ...
    CREATE TABLE jids (
      ...
      load  text NOT NULL,
      alter_time timestamp with time zone NOT NULL DEFAULT now(),
    );
    ...
    CREATE TABLE salt_returns (
      ...
      CONSTRAINT salt_returns_pkey PRIMARY KEY (jid, id)
    );
...
To use the pgupsert returner, append '--return pgupsert' to the salt command.

.. code-block:: bash

    salt '*' test.ping --return pgupsert
...
.. code-block:: bash
    ...
    salt '*' test.ping --return pgupsert --return_config alternative
...
.. code-block:: bash
    ...
    salt '*' test.ping --return pgupsert --return_kwargs '{"db": "another-salt"}'
...
def __virtual__():
    if not HAS_POSTGRES:
        return False, "Could not import pgupsert returner; psycopg2 is not installed."
...
def returner(ret):
    ...
    try:
        with _get_serv(ret, commit=True) as cur:
            sql = """
            INSERT INTO salt_returns (fun, jid, return, id, success, full_ret)
            VALUES (%s, %s, %s, %s, %s, %s)
            ...
            success = False
            # Runners always return false, for whatever reason. This fixes that.
            if isinstance(cleaned_return["return"], dict):
                success = cleaned_return["return"].get(
                    "success", ret.get("success", success)
                )
            else:
                success = ret.get("success", success)

            if "runner" not in ret["fun"]:
                success = ret.get("success", success)
            # High state runs with failed states should report as "failed"
            # Likewise, commands that return 0 should always be marked as true
            if isinstance(cleaned_return, dict) and "retcode" in cleaned_return:
                success = cleaned_return["retcode"] == 0
            cur.execute(
                sql,
                (
                    ret["fun"],

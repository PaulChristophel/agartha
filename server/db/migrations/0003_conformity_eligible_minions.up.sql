CREATE OR REPLACE FUNCTION conformity_minion_ids()
...
BEGIN
	...
	EXECUTE format($view$
		...
		WHERE a.fun IN ('state.highstate', 'state.apply')
			...
			AND (
				a.success::boolean IS FALSE
				AND (a.full_ret::jsonb ->> 'retcode') = '1'
				AND jsonb_typeof(a.return::jsonb) = 'array'
				AND (a.return::jsonb ->> 0)
					LIKE 'The function %% was started at %% with jid %%'
			) IS NOT TRUE
		ORDER BY a.id, a.alter_time DESC
...
WITH normalized AS (
	...
	FROM vw_salt_highstates
		JOIN conformity_minion_ids() AS eligible
			ON eligible.minion_id = vw_salt_highstates.id
),
results AS (
	SELECT
		normalized.id,

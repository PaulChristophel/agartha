...
// ListSaltMinionPillarKeys func retrieves the list of JSON keys from the materialized view with optional filtering and pagination
//
//	@Summary		Retrieve the list of JSON keys from the salt_minions pillar with optional filtering and pagination
//	@Description	Retrieve the list of JSON keys from the salt_minions pillar with optional filtering and pagination
//	@Tags			SaltMinion
//	@Accept			json
//	@Produce		json
//	@Success		200	{object}	dto.SaltMinionPillarKeyResponse
//	@Failure		400	{object}	httputil.HTTPError400
//	@Failure		401	{object}	httputil.HTTPError401
//	@Failure		404	{object}	httputil.HTTPError404
//	@Failure		500	{object}	httputil.HTTPError500
//	@router			/api/v1/salt_minion/pillar_keys [get]
...
//	@Security		Bearer
func ListSaltMinionPillarKeys(c *gin.Context) {
	log := logger.GetLogger()
	var pillarKeys []string
	...
	log.Info("Received request to get salt minions pillar keys",
	...
	query := db.DB.Table("mat_salt_minions_pillar_keys")
	...
	// Execute the query to fetch pillar keys
	if err := query.Find(&pillarKeys).Error; err != nil {
		log.Error("Failed to fetch salt minions pillar keys", zap.Error(err))
		httputil.NewError(c, http.StatusInternalServerError, "Failed to fetch salt minions pillar keys.")
		return
	}

	// If no pillar keys are present, return an error
	if len(pillarKeys) == 0 {
		log.Debug("No pillar keys present in the materialized view")
		httputil.NewError(c, http.StatusNotFound, "No pillar keys present in the materialized view.")
		return
	}
	...
	response := dto.SaltMinionPillarKeyResponse{
		Paging:  paging,
		Results: pillarKeys,
	}

	log.Debug("Successfully retrieved salt minions pillar keys", zap.Int("count", len(pillarKeys)))
